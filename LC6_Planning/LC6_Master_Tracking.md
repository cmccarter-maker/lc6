...
| S22-CASCADE-ASYMMETRIC | RESOLVED S24 | Investigation complete — misdiagnosis, engine is correct | S24 code audit revealed the original "asymmetric cascade is wrong" framing was based on incomplete understanding of the existing code. Engine actually handles terminal-saturation physics correctly via three mechanisms that coexist: (1) bidirectional Washburn wicking at lines 779-797 moves liquid outward when fill gradient exists — already symmetric when physics demands it; (2) inward overflow cascade at 772-777 correctly models external moisture pressure (condensation, ambient `_aHygro`) — one-directional because the driving physics is one-directional; (3) at terminal saturation, `_vaporExitHr = Math.min(sweatGPerHr, eMax/L_V × 3600)` at line 734 captures the user's insight that "drip rate = sweat production − vapor transport" — `_excessHr` IS the drip residual. The silent discard at line 777/857 correctly represents drip loss. Cooling in thermal regulation uses `E_actual` (capped by `eMax`), so at terminal saturation with low VPD, cooling correctly drops to vapor-transport-limited rate — not sweat-production-unlimited. No spec needed. Sustained Socratic physics discussion S24 with user (vapor-still-moves-through-shell-im, drip at skin-base interface, drip-as-residual) converged on the engine's existing approach. See S24 session notes for discussion. |
| S22-MICROCLIMATE-VP | HIGH | Open — spec pending (S25 target), scope sharpened S24 | When shell `im` throttles vapor flux outward (humid ambient, saturated conditions), microclimate VP inside clothing rises. Rising microclimate VP reduces skin-to-microclimate VPD, which throttles evaporation at skin itself. **S24 code audit identified specific manifestation:** `getDrainRate` at `calc_intermittent_moisture.ts:808-809` uses ambient `humidity` parameter, not microclimate humidity at shell inner surface. At terminal fabric saturation, microclimate RH approaches 100%, driving shell drain toward zero — but engine continues computing drain against ambient RH. Result: engine overstates evaporative cooling at terminal saturation in all regimes. Worst in humid-ambient scenarios where ambient and microclimate RH both approach 100% (small effect) but still matters in cold-dry saturated scenarios where ambient RH is low but microclimate RH is high (large effect). Fix scope: thread microclimate VP as engine state, compute shell-drain VPD using microclimate-side VP rather than ambient-side VP. Also affects `computeEmax` at lines 671/702 which currently uses ambient humidity — should use microclimate humidity for skin-to-microclimate gradient, and ambient humidity only for microclimate-to-ambient (shell drain) portion. This is a layered-resistance physics rewrite, not a single formula change. |
...
| S22-HUMIDITY-FLOOR-VALIDATION | RESOLVED S24 | Investigation complete — floor retained | S24 code audit revealed `humidityFloorFactor` has exactly ONE real caller: `waderEvapFloor` in `split_body.ts:160`. The function provides an evaporation floor for WADER scenarios specifically (sets minimum evap rate of 0.005-0.020 depending on RH, scaled down at high humidity). NOT related to Cooper Landing protection as originally assumed. The S12-era Cooper Landing floor was `applyHumidityFloor()` — a different function deleted in Session 11 during hygroscopic absorption physics migration. PHY-HUMID-01 v2 §4.2 claim "redundant with vpdRatio" is incorrect — they operate on different physics (vpdRatio: evap rate multiplier based on VPD; humidityFloorFactor: minimum evap floor for wader numerical safety). Floor remains in place. Separate open question (not blocking): is 0.005 minimum evap at 100% RH inside waders physically defensible? Flagged as `S24-WADER-EVAP-FLOOR-QUESTION` for future consideration but not urgent. |

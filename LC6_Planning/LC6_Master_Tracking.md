...
- **[S30 commit SHA]:** Three-file doc commit on `session-13-phy-humid-v2`
...
| PHY-031 | LC6-v1 (LC6_Planning/specs/PHY-031_Spec_v1_RATIFIED.md) | RATIFIED (spec) / PARTIAL (engine — infrastructure complete S29, physics gap pending S30) | S28 spec ratification (3a4b4d1); S29 engine port (7289e8b) | LC6_Planning/specs/PHY-031_Spec_v1_RATIFIED.md; LC6_Planning/audits/S27_PHY-031_PORT_STATUS_AUDIT.md | Component cycle model ratified LC6 S28 (April 21, 2026). 616-line spec ports LC5 PHY-030/031 ratified values verbatim. Adds S28-ratified content: Thanksgiving holiday window (#10), powder-day auto-detection, powder tier-bump table. **S29 port landed 7289e8b**: phy031_constants.ts + activities/crowd_factor.ts (getCrowdFactor + computeCycle), ActivitySpec.date_iso required, UserBiometrics.ski_history optional, evaluate.ts:430 null-plug closed via computeResortCycleOverride. Cardinal Rule #8 honored (calc_intermittent_moisture.ts untouched). Spec-lock tests 31/31 green. **S29 PHYSICS GAP DISCOVERED**: spec §2.1 cycleMin is full wall-clock cycle time (16.25–50 min depending on terrain+tier+powder) but engine phase loop simulates only run+lift (~10 min). Override supplies totalCycles=N; engine runs N × 10-min physics over session whose wall-clock expects N × cycleMin, under-simulating up to 80% of session time. Root cause of regressed peak_MR post-port. **S30 target**: author PHY-031-CYCLEMIN-RECONCILIATION spec to specify phase decomposition, per-phase physics model, rest handling, history-override decomposition. Implementation session follows S30 ratification (Cardinal Rule #8 active, hand-comp trace gate). Tracker refs: B.18 S27-DRIFT-3-PHY-031-NO-SPEC **CLOSED S29** (moved to Section F); new B.18 rows `S29-PHY-031-CYCLEMIN-PHYSICS-GAP` HIGH and `S30-SESSIONA-DEFERRED` MEDIUM. |
...
| S29-PHY-031-CYCLEMIN-PHYSICS-GAP | HIGH | HIGH-ratified-implementation-pending — S31 target | Spec-implementation gap discovered via S29 4-scenario matrix run. PHY-031 spec §2.1 defines cycleMin as full wall-clock cycle time (16.25–50 min depending on terrain × crowd tier × powder flag). Engine phase loop in calc_intermittent_moisture.ts simulates only run+lift phase durations (~10 min). When `cycleOverride.totalCycles=N` is supplied, engine runs N × 10-min physics cycles over a session whose wall-clock expects N × cycleMin, leaving up to 80% of session time unsimulated. Post-port MR regresses instead of closing the S26 under-estimation. Field validation: 36 × 10-min cycles in 6hr groomer day implies 40-50K feet of vertical at reference 1000 vft/run — physically impossible, and inconsistent with user's decade of snowboarding field observation. **Cardinal Rule #8 exposure**: implementation will touch engine phase-loop semantics — highest-risk patch since PHY-068. **S30 authors** `PHY-031-CYCLEMIN-RECONCILIATION_Spec_v1_DRAFT.md` specifying phase decomposition rules, per-phase physics model, session-level rest handling, history-override decomposition, hand-comp verification vectors (3 scenarios with full calculation trace), §8.5 non-cycleMin `cycleOverride` field inventory for drift detection. **Blocks** `S26-SYSTEMATIC-MR-UNDERESTIMATION` closure. **Blocks** push of `session-13-phy-humid-v2` to origin. Reference: S29 status block narrative; commit 29e0b30. **S30 UPDATE:** Reconciliation spec ratified at `LC6_Planning/specs/PHY-031-CYCLEMIN-RECONCILIATION_Spec_v1_RATIFIED.md` ([S30 commit SHA]). Status flipped to HIGH-ratified-implementation-pending. S31 authorized to execute engine port. Cardinal Rule #8 gate: §9 hand-comp vectors (G1 sessionMR 2.6, M2 sessionMR 4.3, P5 sessionMR 5.5) with ±0.3 tolerance + 8 gate criteria in §9.5 must pass before patch lands. Interface extension mandated (no signature change to calcIntermittentMoisture) per spec §8.6. |
| S29-MATRIX-PENDING | MEDIUM | Open — post-reconciliation re-author | 4-scenario verification matrix (S1 Breck groomers 16F Tue, S2 Breck moguls 37F Tue LC5 parity 6.2, S3 Tahoe bowls 28F 70%RH powder snowboard, S4 Breck groomers Thanksgiving Fri) drafted during S29 as `s29_matrix.test.ts` but file deleted pre-commit after 3/4 scenarios revealed the cyclemin physics gap. Re-author post-reconciliation with: (a) verified fixture ensemble — current fixture was contrived (hand-picked CLO/im) rather than gear-DB products matching LC5 Mar 24 baseline; (b) verified metric read — test read `four_pill.your_gear.trajectory_summary.peak_MR` but LC5 baseline "MR 6.2" was `sessionMR` (session-cumulative, not instantaneous peak); these are different metrics by definition. S29 halted matrix commit per OQ-S29-6 resolution ("halt on out-of-range") rather than loosen ranges. Re-authoring is post-S30 implementation session scope. |
| S30-SESSIONA-DEFERRED | MEDIUM | Deferred pending S30 close | `SessionA_kickoff.md` drafted S28 close, never executed. Per user decision at S30 open (April 22, 2026), SessionA is held until after S30 implementation chain completes (S30 spec → S30+ implementation → S-001 regression closed → branch pushed). Kickoff file installed at `LC6_Planning/session_kickoffs/SessionA_kickoff.md` in commit 435e321. **Do not open SessionA before S-001 regression closes** — audit value grows with more data; premature audit would catalog an in-progress state. Reference: SessionA scope is cross-package port-state snapshot (`SA_LC6_State_of_the_Port.md`) + failure-mode catalog (`SA_Failure_Mode_Catalog.md`). |

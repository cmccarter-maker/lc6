...
| PHY-031 | Component Cycle Model (crowd tiers, holiday windows, line + transition + rest) | RATIFIED | PARTIAL | LC6 spec: LC6_Planning/specs/PHY-031_Spec_v1_RATIFIED.md. Engine (S29): phy031_constants.ts + activities/crowd_factor.ts (getCrowdFactor + computeCycle), ActivitySpec.date_iso required, UserBiometrics.ski_history optional, evaluate.ts:430 null-plug closed via computeResortCycleOverride helper. Cardinal Rule #8 honored (calc_intermittent_moisture.ts untouched). **S29 PHYSICS GAP DISCOVERED:** spec §2.1 cycleMin is full wall-clock cycle time (16.25–50 min depending on terrain + tier + powder) but engine phase loop simulates only run+lift (~10 min). Override supplies totalCycles=N to engine, engine runs N × 10-min physics over session whose wall-clock expects N × cycleMin, under-simulating up to 80% of session time. MR regresses post-port. Blocking S26-SYSTEMATIC-MR-UNDERESTIMATION closure. Tracked as `S29-PHY-031-CYCLEMIN-PHYSICS-GAP` HIGH-ratified-implementation-pending; reconciliation spec ratified S30 at LC6_Planning/specs/PHY-031-CYCLEMIN-RECONCILIATION_Spec_v1_RATIFIED.md. S31 engine implementation authorized; hand-comp vectors at spec §9 are the Cardinal Rule #8 gate before patch lands. | 31 test refs (spec-locks/phy-031-component-cycle.test.ts; 31/31 green) | [S30 commit SHA] (S30) | **Spec values + calendar + schema correct. Reconciliation spec ratified S30, engine implementation authorized S31. PHY-031 row remains PARTIAL until S31 port lands.** |
| PHY-031-CYCLEMIN-RECONCILIATION v1 | Engine consumption of PHY-031 component cycle duration (phase decomposition, per-phase physics, session-level rest handling) | RATIFIED | DRAFT (engine — S31 target) | LC6_Planning/specs/PHY-031-CYCLEMIN-RECONCILIATION_Spec_v1_RATIFIED.md (spec only; no engine implementation yet) | 0 test refs (S31 scope; §9 hand-comp vectors are binding regression anchors) | [S30 commit SHA] | No — spec ratified S30, engine implementation authorized for S31, Cardinal Rule #8 gate at spec §9.5. |
